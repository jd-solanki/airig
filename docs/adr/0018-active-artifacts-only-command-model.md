...
Remote `.ai/` content is active setup source, not a downloaded release cache. For remote Setup Releases, ohmyai writes only selected active artifacts into `.ai/`, plus any relative symlink dependency needed to preserve an Author-provided setup symlink such as `CLAUDE.md -> AGENTS.md`.
